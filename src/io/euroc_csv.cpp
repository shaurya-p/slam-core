...
std::ifstream open_or_throw(const std::filesystem::path& path, const char* what) {
    std::ifstream f(path);
    if (!f.is_open()) {
...
std::vector<imu::ImuMeasurement> read_euroc_imu_csv(const std::filesystem::path& path,
    ...
    if (samples.empty()) {
        throw std::invalid_argument("EuRoC IMU CSV has no parseable rows: " + path.string());
    }
    return samples;
}

std::vector<EurocGtSample> read_euroc_gt_csv(const std::filesystem::path& path, int* skipped_rows) {
    ...
    while (std::getline(f, line)) {
        ...
        const double norm = std::sqrt(q_w * q_w + q_x * q_x + q_y * q_y + q_z * q_z);
        if (norm < 1e-10) {
            ++skipped;
            continue;
        }
        ...
        s.q_W_B       = Eigen::Quaterniond(q_w / norm, q_x / norm, q_y / norm, q_z / norm);
    ...
    if (skipped_rows != nullptr) *skipped_rows = skipped;
    if (samples.empty()) {
        throw std::invalid_argument("EuRoC GT CSV has no parseable rows: " + path.string());
    }
    return samples;
}
...
const EurocGtSample& nearest_gt(double timestamp_s, const std::vector<EurocGtSample>& samples) {
    ...
    auto it = std::lower_bound(samples.begin(), samples.end(), timestamp_s,
    ...
    return (timestamp_s - prev->timestamp_s <= it->timestamp_s - timestamp_s) ? *prev : *it;
}

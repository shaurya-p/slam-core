    ...
    double dt_s)
{
    if (!std::isfinite(dt_s) || dt_s <= 0.0) {
        throw std::invalid_argument(
            "propagate_gyro: dt_s must be positive and finite");
    }
    // R_W_B_next = R_W_B * exp_so3(gyro_radps * dt_s)
    // gyro_radps is body-frame, so the rotation increment is right-multiplied.
    return R_W_B * slam_core::geometry::exp_so3(gyro_radps * dt_s);
}
    ...
    double dt_s)
{
    if (!std::isfinite(dt_s) || dt_s <= 0.0) {
        throw std::invalid_argument(
            "propagate_gyro_bias_corrected: dt_s must be positive and finite");
    }

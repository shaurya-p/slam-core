...
static bool imu_state_is_finite(const ImuState& s) {
    return std::isfinite(s.timestamp_s)
        && s.R_W_B.allFinite()
        && s.p_W_B.allFinite()
        && s.v_W_B.allFinite()
        && s.gyro_bias_radps.allFinite()
        && s.accel_bias_mps2.allFinite();
}
    ...
    double                 dt_s)
{
    if (!std::isfinite(dt_s) || dt_s <= 0.0) {
        throw std::invalid_argument(
            "propagate_imu_state: dt_s must be positive and finite");
    }
    if (!imu_state_is_finite(state)) {
        throw std::invalid_argument(
            "propagate_imu_state: state contains non-finite values");
    }
    if (!is_finite(measurement)) {
        throw std::invalid_argument(
            "propagate_imu_state: measurement contains non-finite values");
    }
    if (!gravity_W.allFinite()) {
        throw std::invalid_argument(
            "propagate_imu_state: gravity_W contains non-finite values");
    }

...
TEST(ImuStatePropagation, GravityDisabledZeroAccelKeepsVelocityAndPosition) {
    ...
    const ImuState next =
        propagate_imu_state(state, make_meas(), Eigen::Vector3d::Zero(), 0.01);
...
TEST(ImuStatePropagation, RotatedBodyAccelerationIntoWorld) {
    ...
    const ImuState next =
        propagate_imu_state(state, meas, Eigen::Vector3d::Zero(), 1.0);
...
TEST(ImuStatePropagation, GyroBiasAffectsOrientation) {
    ...
    const ImuState next =
        propagate_imu_state(state, meas, Eigen::Vector3d::Zero(), 2.0);
...
TEST(ImuStatePropagation, TimestampAdvancesByDt) {
    ...
    const ImuState next =
        propagate_imu_state(state, make_meas(), Eigen::Vector3d::Zero(), dt);
...
TEST(ImuStatePropagation, NonFiniteInputsThrow) {
    ...
    {
        ImuState s = make_state(); s.p_W_B.x() = kNaN;
        EXPECT_THROW(propagate_imu_state(s, make_meas(), g, dt), std::invalid_argument);
    }
    {
        ImuState s = make_state(); s.v_W_B.y() = kNaN;
        EXPECT_THROW(propagate_imu_state(s, make_meas(), g, dt), std::invalid_argument);
    }
    {
        ImuState s = make_state(); s.gyro_bias_radps.z() = kNaN;
        EXPECT_THROW(propagate_imu_state(s, make_meas(), g, dt), std::invalid_argument);
    }
    {
        ImuState s = make_state(); s.accel_bias_mps2.x() = kNaN;
        EXPECT_THROW(propagate_imu_state(s, make_meas(), g, dt), std::invalid_argument);
    }
    {

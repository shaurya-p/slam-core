...
static std::vector<ImuMeasurement> make_stationary_meas(
    const Eigen::Vector3d& accel, int n = 10)
{
...
TEST(ImuInitialization, NoisyStationary)
{
    ...
    for (const auto& a : noisy_accels) { meas.push_back(make_meas(a)); }
    ...
    for (const auto& m : meas) { accel_sum += m.accel_mps2; }
...
{
    EXPECT_THROW(
        estimate_R_W_B_from_stationary({}, kGravityW),
...
TEST(ImuInitialization, InvalidSingleMeasurement)
{
    ...
    EXPECT_THROW(
        estimate_R_W_B_from_stationary(meas, kGravityW),
        std::invalid_argument);
}
...
TEST(ImuInitialization, InvalidNonFiniteGravity)
{
    ...
    EXPECT_THROW(
        estimate_R_W_B_from_stationary(meas, Eigen::Vector3d(0.0, 0.0, nan)),
        std::invalid_argument);
}
...
TEST(ImuInitialization, InvalidNearZeroGravity)
{
    auto meas = make_stationary_meas({0.0, 0.0, 9.81});
    EXPECT_THROW(
        estimate_R_W_B_from_stationary(meas, Eigen::Vector3d(0.0, 0.0, 1e-10)),
        std::invalid_argument);
}

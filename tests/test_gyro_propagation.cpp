...
TEST(GyroPropagation, ConstantZGyro90DegIn1s) {
    ...
    const Eigen::Matrix3d result   = propagate_gyro(Eigen::Matrix3d::Identity(), gyro, 1.0);
...
TEST(GyroPropagation, ManySmallStepsMatchOneLargeStep) {
    ...
    const Eigen::Matrix3d R_one = propagate_gyro(Eigen::Matrix3d::Identity(), gyro, total_dt);
...
TEST(GyroPropagationBiasCorrected, ZeroBiasMatchesPropagateGyro) {
    const Eigen::Matrix3d R = exp_so3({0.3, -0.5, 0.8});
    const Eigen::Vector3d gyro{0.1, -0.2, 0.3};
    const Eigen::Matrix3d result   = propagate_gyro(R, gyro, 0.01, Eigen::Vector3d::Zero());
...
TEST(GyroPropagationBiasCorrected, FullBiasLeavesOrientationUnchanged) {
    ...
    const Eigen::Matrix3d result = propagate_gyro(R, gyro, 0.05, gyro);
    EXPECT_TRUE(result.isApprox(R, 1e-12));
}
...
TEST(GyroPropagationBiasCorrected, PartialBiasProducesExpectedRotation) {
    ...
    const Eigen::Matrix3d result = propagate_gyro(Eigen::Matrix3d::Identity(), gyro, 2.0, bias);

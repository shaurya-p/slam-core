...
int main(int argc, char* argv[]) {
    ...
    while (window_start_t + window_duration <= gt_end) {
        ...
        if (integrated_dt <= 0.0) {
            window_start_t += window_duration;
            continue;
        }

        // GT velocity change at window boundaries.
        const Eigen::Vector3d dv_gt = slam_core::io::nearest_gt(t1, gt_data).v_W_B -
        ...
        const Eigen::Vector3d dv_err       = (dv_raw + gravity_W * integrated_dt) - dv_gt;
        const Eigen::Vector3d accel_bias_B = M.colPivHouseholderQr().solve(dv_err);

        out_file << t0 << ',' << t1 << ',' << integrated_dt << ',' << accel_bias_B.x() << ','
                 << accel_bias_B.y() << ',' << accel_bias_B.z() << ',' << accel_bias_B.norm() << ','
                 << dv_err.norm() << '\n';
    ...
    const double max_vel_err = *std::max_element(vel_error_norms.begin(), vel_error_norms.end());
